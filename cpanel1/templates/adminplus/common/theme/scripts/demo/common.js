...
$(function()
{
	...
	var navH = $('.nav-container').innerHeight();
	$('.navbar.main .btn-navbar').click(function()
	{
		...
		*/
		...
		if($('.nav-container').is(':hidden') && $('.navresponsive-controll').is(':hidden'))
		{
			$('.nav-container:hidden').css({'display':'block', 'height':0}).animate({height:navH}, 'fast', function()
			{
				$(this).css({'height':'auto'});
			});
		}
		else if(!$('.nav-container').is(':hidden') && !$('.navresponsive-controll').is(':hidden'))
		{
			$('.nav-container:not(:hidden)').animate({height:0}, 'fast', function()
			{
	...
	$('.checkboxs tbody').on('click', 'tr.selectable', function(e){
		...
		if (e.srcElement.nodeName != 'TD' && e.srcElement.nodeName != 'TR' && e.srcElement.nodeName != 'DIV')
		{
			return true;
		}
		else
	...
	{	
		...
		{	
		    ...
		    stop: function(e, ui) { if (typeof mainYScroller != 'undefined') mainYScroller.enable();

				var chkItemList = "";
				$('.js-table-sortable input[class=chkItem]').each(function(i){
		            chkItemList = chkItemList + ',' + $(this).val();
		        });

				$.ajax({
					url: 'http://cms4.demo/cpanel_cms4/user_order.ajax',
					type: 'post',
					dataType: 'json',
					data: { hashcode: hashcode, chkItemList: chkItemList },
		...
		        });
				...
				//alert( $(this).find('#chkItem:checkbox').val() );
				//alert(ui.item.index());
				//alert( $('.js-table-sortable input[class=ordItem]').eq(ui.item.index()+1).val() );
				//alert( $( ".selectable" ).item.find(".uniformjs #chkItem:checkbox").val());
				//alert(ui.item.find('.chkItem:checkbox').val());
				*/
				
				/* lay 2 dong ke nhau trong table
				var id_first 	= ui.item.find('.chkItem:checkbox').val();
				var order_first = ui.item.find('.ordItem:hidden').val();
				
				var id_last 	= $('.js-table-sortable input[class=chkItem]').eq(ui.item.index()+1).val();
				var order_last 	= $('.js-table-sortable input[class=ordItem]').eq(ui.item.index()+1).val();	

				$.ajax({
					url: 'http://cms4.demo/cpanel_cms4/user_order.ajax',
					type: 'post',
					dataType: 'json',
					data: { hashcode: hashcode, id_first: id_first, order_first: order_first, id_last: id_last, order_last: order_last },
					success: function (data) {
						//var msg = $.parseJSON(data);
						//alert(data.msg);
					}
