<?php defined( '_VALID_MOS' ) or die( include("404.php") );
    ...
    {
	    function process_add_category($danhmuc_id, $danhmuccha, $tieude, $alias, $tomtat ,$image_src, $ngaythem, $ngaysua, $thutu, $hienthi, $luotxem, $seo_tieudetrang, $seo_thetukhoa, $seo_themota)
        {			
		    $sql = "INSERT INTO danhmucgiaodien(
						danhmucgiaodien.danhmuc_id,
						danhmucgiaodien.danhmuccha,
						danhmucgiaodien.tieude,
						danhmucgiaodien.alias,
						danhmucgiaodien.tomtat,
						danhmucgiaodien.hinhanh,
						danhmucgiaodien.ngaythem,
						danhmucgiaodien.ngaysua,
						danhmucgiaodien.thutu,
						danhmucgiaodien.hienthi,
						danhmucgiaodien.luotxem,
						danhmucgiaodien.seo_tieudetrang,
						danhmucgiaodien.seo_thetukhoa,
						danhmucgiaodien.seo_themota)
				    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

            if ($this->dbObj->SqlQueryInputResult($sql, array($danhmuc_id, $danhmuccha, $tieude, $alias, $tomtat ,$image_src, $ngaythem, $ngaysua, $thutu, $hienthi, $luotxem, $seo_tieudetrang, $seo_thetukhoa, $seo_themota)) <> FALSE) {
                return true;
            } else {
                return false;
            }
        ...
        case "category.add";
			...
			if($validator->ValidateForm())
			{
				if($_POST["act"] == "save"){
                    ...
                    $temp_image_src         = explode('/',$_POST["image_src"]);
                    $image_src 			    = end($temp_image_src);
					$category_desc 			= $_POST["category_desc"];
					$date_add				= $func->_formatdate($_POST["date_add"]);
					$date_edit				= $func->_formatdate($_POST["date_add"]);
					$sort					= $func->process_getmaxid("danhmucgiaodien", "thutu");			
					...
					if($category_process->process_add_category($category_id, $parent_category, $category_title, $alias, $category_desc,$image_src, $date_add, $date_edit, $sort, $category_display, $num_view, $category_seo_title, $category_seo_keyword, $category_seo_desc) <> FALSE){
						...
						$func->_redirect($index_backend . "template/category/add.html");
						exit;
					} else {
