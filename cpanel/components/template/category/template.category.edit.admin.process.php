    ...
    class category_process extends template
    {
		...
		public function get_category_edit( $danhmuc_id )
	    {
		    $sql = "SELECT 
					...
					WHERE danhmucgiaodien.danhmuc_id = ?";
		    $result = $this->dbObj->SqlQueryOutputResult($sql, array( $danhmuc_id ));
		    return $result;
	    }		
        
	    function process_edit_category($danhmuccha, $tieude, $tomtat,$hinhanh, $ngaysua, $hienthi, $seo_tieudetrang, $seo_thetukhoa, $seo_themota, $danhmuc_id)
        {			
		    $sql = "UPDATE danhmucgiaodien SET 
						danhmucgiaodien.danhmuccha = ?,
						danhmucgiaodien.tieude = ?,
						danhmucgiaodien.tomtat = ?,
						danhmucgiaodien.hinhanh = ?,
            ...
            if ($this->dbObj->SqlQueryInputResult($sql, array($danhmuccha, $tieude, $tomtat,$hinhanh, $ngaysua, $hienthi, $seo_tieudetrang, $seo_thetukhoa, $seo_themota, $danhmuc_id)) <> FALSE) {
                return true;
            } else {
                return false;
            }
        ...
        case "category.edit";
			...
			if($validator->ValidateForm())
			{
				if($_POST["act"] == "save"){
					...
					if($category_process->process_edit_category($parent_category, $category_title, $category_desc,$image_src, $date_edit, $category_display, $category_seo_title, $category_seo_keyword, $category_seo_desc, $category_id) <> FALSE){
						...
						$func->_redirect($index_backend . "template/category/edit/$category_id.html");
						exit;
