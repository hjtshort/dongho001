    ...
    class news_process extends content_model
    {
	    ...
	    {		
		    ...
		    $query = "SELECT 
						...
						tintuc.tinnoibat,
						tintuc.solanxem,
						(Select GROUP_CONCAT(' ', danhmuctintuc.tieude)
                                               FROM danhmuctintuc
                                               WHERE  FIND_IN_SET(danhmuctintuc.danhmuc_id, tintuc.danhmuc_id)) as danhmuctieude
					FROM tintuc
					INNER JOIN danhmuctintuc ON danhmuctintuc.danhmuc_id IN (SELECT danhmuctintuc.danhmuc_id FROM danhmuctintuc WHERE FIND_IN_SET(danhmuctintuc.danhmuc_id, tintuc.danhmuc_id))
					GROUP BY Id
					HAVING tintuc.tieude LIKE ? $condition 
					ORDER BY  tintuc.ngaysua DESC,tintuc.thutu ASC
					LIMIT $offset, $limit";
		    $result = $this->dbObj->SqlQueryOutputResult($query, array( $title ));
		    return $result;
	    }				
