<?php 
	...
	class process  extends interface_model
    {
        ...
        function __construct()
        {
            ...
            $this->Id_template = $template_id = empty($conf['geturl']['id'])?$conf['app']['template']:$conf['geturl']['id'];
            $this->path_template = REAL_PATH . "/" . $conf['data_user'] . "/templates/$template_id";
        }
        function data_setting_template(){
            $data_setting_template = array();
            $path_template = $this->path_template;
            if (file_exists("$path_template/config/setting_data.json")) {
                $data_json = file_get_contents("$path_template/config/setting_data.json");
                $data_setting_template = json_decode($data_json, true)['data'];
            }
            return $data_setting_template;
        }
        ...
        public function get_list_group_menu_json()
        {
            $query = "SELECT 
						Id,title
					FROM nhommenu WHERE activated = 1";
    ...
    switch(@$_POST["hidden"]){
		...
		case "save":
			...
			$data_old = $myprocess->data_setting_template();
			$data 	  = array();
			...
			if (file_put_contents("$path_template/config/setting_data.json", $func->black_list_tags($json)))
            {
